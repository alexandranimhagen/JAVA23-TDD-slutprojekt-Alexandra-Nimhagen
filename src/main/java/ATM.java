...
public class ATM {
    ...
    // Sätt vilken bank som denna ATM är kopplad till
...
// Metod för att hantera misslyckade PIN-försök
    ...
    protected void handleDeposit(Scanner scanner) {
    ...
    protected void handleWithdraw(Scanner scanner) {
    ...
    private double getValidAmount(Scanner scanner, String prompt, int minAmount, int step) {
        double amount = -1;
        while (true) {
            try {
                System.out.print(prompt);
                amount = scanner.nextDouble(); // Läs in belopp
                if (amount < minAmount) {
